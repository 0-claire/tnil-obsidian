...
export class TNILUI {
    ...
    parseOption(option: (typeof this.options)[number] | keyof SearchSettings['fields'] | keyof SearchSettings["settings"], searchSettingsState: ReturnType<typeof useState<SearchSettings>>) {
        ...
        if(this.options.includes(finalOption))
            searchSettings?.searchType = finalOption
        else if (option in searchSettings?.fields) {
            searchSettings?.fields[finalOption] = optionValue;
